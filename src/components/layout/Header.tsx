...
const navLinks = [
  { name: "ראשי", path: "#hero" },
  { name: "למה הקורס הזה?", path: "#why" },
  { name: "הקורס", path: "#course" },
    ...
    >
      <div className="container mx-auto px-4">
        <nav className="flex items-center justify-between">
          <a href="#hero" onClick={(e: any) => scrollToSection(e, "#hero")} className="flex items-center gap-3 group">
          ...
          <div className="hidden md:flex items-center space-x-8 space-x-reverse">
            ...
            <Button asChild className="rounded-full px-6 gradient-brand shimmer shadow-sm">
              <a href="#contact" onClick={(e: any) => scrollToSection(e, "#contact")} className="text-white">
                הרשמה לקורס
              </a>
            </Button>
          </div>
          ...
          <div className="md:hidden absolute top-full left-0 right-0 bg-white border-t p-4 space-y-4 shadow-xl animate-in fade-in slide-in-from-top-4">
            {navLinks.map((link) => (
              <a
                ...
                onClick={(e: any) => scrollToSection(e, link.path)}
              >
                {link.name}
              </a>
            ))}
            <Button asChild className="w-full rounded-full gradient-brand shimmer shadow-md py-6 text-lg">
              <a href="#contact" onClick={(e: React.MouseEvent<HTMLAnchorElement>) => scrollToSection(e, "#contact")} className="text-white flex items-center justify-center">
